...
import type { DesignSkill, DesignSkillCreate, Product } from '../types';
...
export default function DesignSkillsPage() {
  ...
  async function load() {
    const [skillRows, productRows] = await Promise.all([designSkillApi.list(), productApi.list()]);
    setSkills(skillRows);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await designSkillApi.create(form);
      setForm(initial);
      await load();
    } catch (err) {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <form className="panel p-6 space-y-4" onSubmit={submit}>
        ...
        <button className="btn-primary" disabled={saving}>{saving && <Loader2 className="animate-spin" size={16} />}保存 Skill</button>
      </form>
      <div className="panel overflow-hidden">
        ...
        <div className="divide-y divide-[var(--border)] max-h-[780px] overflow-auto">
          ...
          {skills.map((skill) => <div key={skill.id} className="px-5 py-4 flex justify-between gap-3">
            ...
            <button className="text-red-600 shrink-0 p-1" onClick={() => remove(skill.id)}><Trash2 size={16} /></button>
          </div>)}
        </div>
      </div>
    </div>
